...
notifBtn.onclick = async (e) => {
  ...
  notifications.forEach((n) => {
    const icon =
      n.type === 'like' ? 'fa-heart text-red-500' : 'fa-comment text-blue-500';
    notifList.innerHTML += `
                    <div class="p-4 flex gap-3 ${n.isRead ? 'bg-white' : 'bg-blue-50'} transition border-b">
...
document.getElementById('markAllRead').onclick = async () => {
  ...
  notifList.innerHTML =
    '<p class="p-6 text-center text-gray-500">All read! 🎉</p>';
